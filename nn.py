...
class node(prop):
    ...
    def frwd_prop(self, inp):
        ...
        return [numpy.array([f.func(j) for f, j in zip(self.funcs, self.mask * inp)]), ]
    ...
    def back_prop(self, err):
        return [numpy.array([f.derv(j) for f, j in zip(self.funcs, self.mask * self.inp)] * err), ]
...
class node_out(prop):
    ...
    def frwd_prop(self, inp):
        super().frwd_prop(inp)
        return numpy.array(self.func.func(inp))

    def back_prop(self, tar):
        return [numpy.array(self.func.derv(self.inp, tar)), ], self.func.cost(self.inp, tar)
...
class fc(prop):
    ...
    def frwd_prop(self, inp):
        ...
        super().frwd_prop(inp)
        return [numpy.dot(self.weight, inp), ]
    ...
    def back_prop(self, err):
        ...
        return [numpy.dot(self.weight.T[:-1], err), ]
...
class cnvl(prop):
    ...
    def show_kernel(self):
        for kernel in self.kernel:
            # plt.hist(kernel.flatten(), bins=10)
            # plt.show()
...
class deploy(prop):
    ...
    def frwd_prop(self, inp):
        super().frwd_prop(inp)
        return [numpy.array(inp).flatten(), ]

    def back_prop(self, err):
        return [numpy.array(err).reshape(self.inp.shape), ]
